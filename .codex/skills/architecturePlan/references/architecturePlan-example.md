...
| KPI 제목 | 연결 목표 | 측정 구간 | 목표값 |
| --- | --- | --- | --- |
| 사용자 대상 서비스 가용성 | 장애 영향 제한 | 월간 | 99.9% 이상 |
| 사용자 요청 응답시간 p95 | 안정적인 요청 처리 | 월간 운영 트래픽 | 500ms 이하 |
| 운영 변경 구간 서버 오류 응답 비율 | 정상 상태 복구 | 운영 변경 진행 구간 | 1% 미만 |
| 심각 장애 최초 알림 지연 | 장애 인지와 요청 추적 | 장애 건별 | 5분 이내 |
| 서비스 복구 시간 | 정상 상태 복구 | 분기별 복구 훈련 | RTO 30분 이내 |
| 복구 데이터 손실 구간 | 정상 상태 복구 | 분기별 복구 훈련 | RPO 5분 이내 |
| 표본 요청 처리 경로 재구성 성공률 | 장애 인지와 요청 추적 | 릴리스별 20건 | 100% |
| 환경 격리 정책 위반 경로 수 | 환경 격리 | 릴리스별 구성 검사 | 0개 |
...
| KPI 제목 | 검증 절차 | 증적 | 통과 조건 |
| --- | --- | --- | --- |
| 사용자 대상 서비스 가용성 | 서비스 가용성 지표를 월간 집계한다. | 월간 SLO 보고서 | 99.9% 이상 |
| 사용자 요청 응답시간 p95 | 요청 응답시간의 월간 p95를 집계한다. | 성능 대시보드 Snapshot | 500ms 이하 |
| 운영 변경 구간 서버 오류 응답 비율 | 운영 변경 전후 15분의 서버 오류 응답을 비교한다. | 변경별 오류율 보고서 | 변경 구간 1% 미만 |
| 심각 장애 최초 알림 지연 | 장애 주입 또는 실제 장애 발생 시각과 최초 Critical 알림 수신 시각을 비교한다. | 장애·인시던트 타임라인 | 5분 이내 |
| 서비스 복구 시간 | 격리 환경에서 서비스 복구 훈련을 수행한다. | 분기별 복구 훈련 기록 | RTO 30분 이내 |
| 복구 데이터 손실 구간 | 복원 데이터의 마지막 정상 시점을 확인한다. | 분기별 복구 훈련 기록 | RPO 5분 이내 |
| 표본 요청 처리 경로 재구성 성공률 | 표본 요청 20건의 처리 경로를 로그에서 재구성한다. | 릴리스 검증 기록 | 20건 모두 성공 |
| 환경 격리 정책 위반 경로 수 | 환경 간 네트워크·권한 구성을 정책 검사한다. | 릴리스별 구성 검사 결과 | 위반 경로 0개 |
...
| 질문 | 결정 기한 | 담당 | 상태 |
| --- | --- | --- | --- |
| 리전 재해 복구를 Warm Standby로 상향할 것인가? | 2026-09-15 | Platform Lead | OPEN |
| 로그 저장소를 S3 장기 보관과 연계할 것인가? | 2026-09-15 | SRE Lead | OPEN |
