...
- 구현 전 목표 구조를 결정하는 `PRE_IMPLEMENTATION_PLAN`과 구현 후 실제 도메인 구조를 기록하는 `POST_IMPLEMENTATION_ERD`를 명확히 분리한다.
...
- 결정 패키지를 planner에 hand-off하여 ArchitecturePlan 문서로 구조화하게 한다.
- 예상 schema, entity 또는 관계를 구현 완료 Domain ERD로 표현하거나 저장하지 않는다.

## `POST_IMPLEMENTATION_ERD`

- 구현이 완료된 프로젝트의 실제 schema, entity/model과 migration을 기준으로 Domain ERD 스냅샷을 작성할 때 사용한다.
- 계획 문서나 예상 구조가 아니라 현재 소스와 migration으로 확인되는 구조만 포함한다.
- `ArchitectureErdV1` diagram을 doc-curator에 hand-off하여 `kind: "ARCHITECTURE"` 문서로 저장하게 한다.
- 실제 구현 근거가 없거나 서로 충돌하여 현재 구조를 확정할 수 없으면 ERD를 추측하지 않고 `blocked`로 반환한다.
...
- doc-curator가 조회한 기존 Project, ArchitecturePlan, Plan과 관련 프로젝트 문맥
...
- 기존 ArchitecturePlan의 존재 여부, 식별자, 버전과 현재 상태
- 목표 사용자 흐름, 트래픽과 핵심 품질 속성의 우선순위
- 기술·인프라 선택을 제한하는 기존 계약과 승인된 결정
- 변경 가능한 범위, 마이그레이션·호환성 제약과 허용 가능한 운영 비용

## `POST_IMPLEMENTATION_ERD` 필수 입력

- 실제 schema, entity/model과 migration 파일의 경로
- 데이터 저장소별 canonical source와 현재 적용 상태
- 관계를 증명하는 foreign key, relation 선언 또는 동등한 구현 근거
- 가능한 경우 스냅샷을 연결할 source revision
...
2. 기존 ArchitecturePlan을 확인하여 신규 생성과 업데이트 중 필요한 동작을 식별하고, 관련 Plan 변경과의 정합성을 검토한다.
...
```text
schema + entity/model + migration
                 │
                 ▼
             architect
       ArchitectureErdV1 생성
                 │
                 ▼
            doc-curator
   kind="ARCHITECTURE", diagram 저장
```

1. 실제 schema, entity/model, migration과 적용 상태를 확인한다.
2. canonical source 사이의 차이를 비교하고 현재 구조를 증명할 근거를 기록한다.
3. 실제 field, primary key, foreign key, unique, nullable과 default 정보를 정규화한다.
4. 실제 relation 선언 또는 foreign key로 확인되는 관계와 cardinality만 작성한다.
5. 아래 `ArchitectureErdV1` 계약과 모든 참조 무결성 규칙을 검증한다.
...
- 실제 schema, entity/model 또는 migration 근거가 없으면 Domain ERD를 생성하거나 hand-off하지 않는다.
- `ARCHITECTURE` 저장용 hand-off에는 `kind: "ARCHITECTURE"`와 검증된 `diagram`을 사용한다.
- `ARCHITECTURE` 저장 payload에 `content`, `html`, `taskId` 또는 `planId`를 포함하지 않는다.

</HARD-GATE>

- `docs/architecture/ARCHITECTURE.md`의 최신 `ArchitectureErdV1` 계약을 authoritative source로 사용한다.
- 정의되지 않은 root, entity, field, relationship와 endpoint key를 추가하지 않는다.
- entity의 `id`와 `name`, entity 내부 field의 `name`, relationship의 `id`와 동일 endpoint 쌍은 중복될 수 없다.
- relationship의 `entityId`는 존재하는 entity를 참조해야 하며, `field`가 있으면 해당 entity의 실제 field를 참조해야 한다.
- entity는 최소 1개, entity별 field는 최소 1개가 필요하며 관계가 없는 프로젝트는 빈 `relationships`를 허용한다.
- entity 100개, entity별 field 100개, 전체 field 2,000개와 relationship 1,000개의 상한을 지킨다.
- `generatedAt`은 포함하는 경우 ISO datetime을 사용하고, `sourceRevision`은 확인된 revision만 기록한다.

# 출력과 hand-off 계약

## 공통 출력

- `mode`: `PRE_IMPLEMENTATION_PLAN` 또는 `POST_IMPLEMENTATION_ERD`
...
- 프로젝트 식별자와 ArchitecturePlan 제목
- 기존 ArchitecturePlan 식별자와 `create` 또는 `update` 권고
...
- 위험, 검증 기준과 최종 승인 상태
- 수신자 `planner`와 ArchitecturePlan 구조화 목적을 명시한 hand-off
...
## `POST_IMPLEMENTATION_ERD` 출력

- 프로젝트 식별자와 Architecture 제목
- 확인한 schema, entity/model, migration 경로와 source revision
- 근거 사이의 충돌과 해소 결과
- 검증을 통과한 `ArchitectureErdV1` diagram
- 수신자 `doc-curator`, `kind: "ARCHITECTURE"`와 `diagram` 저장 목적을 명시한 hand-off

# 에이전트별 책임 경계

- **architect**: 아키텍처 결정, 대안·trade-off 평가, 구조도 명세와 구현 후 Domain ERD 검증을 담당한다.
- **coder**: 코드베이스의 경로, 심볼, 현재 동작과 설정 근거를 확인하고 코드를 구현한다.
- **researcher**: 최신 공식 자료, 버전, 지원 범위, 비용, 보안과 외부 제약을 조사한다.
- **planner**: 승인된 아키텍처 결정과 요구사항을 ArchitecturePlan, Plan과 Task 문서로 구조화한다.
- **doc-curator**: 프로젝트 문맥을 조회하고 완성된 Markdown·HTML 또는 diagram을 MCP 문서로 저장하며 저장 결과를 검증한다.
...
- 모든 entity, field와 relationship이 실제 schema, entity/model 또는 migration 근거에 연결되어 있다.
- 계획된 구조나 추측한 관계가 포함되지 않았다.
- `ArchitectureErdV1` 스키마, 참조 무결성, 중복과 상한 검증을 통과했다.
